...
const CameraMonitoring = ({ roverId, substationId }) => {
  ...
  useEffect(() => {
    if (image && canvasRef.current) {
      ...
      img.onload = () => {
        ...
        // Dimensões do container (parent do canvas)
        // Subtraindo ~40 de altura para dar espaço ao slider (se necessário)
        const containerWidth = canvas.parentElement.clientWidth
        const containerHeight = canvas.parentElement.clientHeight - 40

        // Rotacionada 90°: a "largura" da imagem rotacionada é originalHeight
        // e a "altura" é originalWidth.
        const rotatedWidth = originalHeight
        const rotatedHeight = originalWidth

        // Scale para caber completamente (modo "contain")
        let scale = Math.min(containerWidth / rotatedWidth, containerHeight / rotatedHeight)

        // Se quiser permitir que a imagem preencha o container maior que o tamanho original,
        // NÃO limitamos scale a 1. (removido o if scale > 1)
        // Se quiser impedir que fique maior que o original, descomente:
        // if (scale > 1) scale = 1

        // Ajusta o canvas para o tamanho escalado
        ...
        // Rotaciona 90° para desenhar imagem em pé
        ...
        objectData.forEach((obj) => {
          ...
          const boxWidth = rotatedX2 - rotatedX1
          const boxHeight = rotatedY2 - rotatedY1

          ctx.strokeStyle = '#00FF00'
          ctx.lineWidth = 2
          ctx.strokeRect(rotatedX1, rotatedY1, boxWidth, boxHeight)

          if (obj.tag) {
            ctx.fillStyle = '#00FF00'
            ctx.font = '16px Arial'
            ctx.fillText(obj.tag, rotatedX1, rotatedY1 - 5)
          }
        })
      }
  ...
  const handleCanvasClick = (event) => {
    ...
    objectData.forEach((obj, index) => {
      ...
      if (clickX >= rotatedX1 && clickX <= rotatedX2 && clickY >= rotatedY1 && clickY <= rotatedY2) {
        boxClicked = true
